...
class FollowFiducial(object):
    ...
    def get_fiducial_objects(self, fiducial_id):
        """Get a specific fiducial that Spot detects with its perception system by ID."""
        ...
        for fiducial in fiducial_objects:
            if fiducial.apriltag_properties.tag_id == fiducial_id:
                return fiducial
        # Return none if no fiducial with the given ID is found.
        return None
    ...
    def offset_tag_pose(self, object_rt_world, dist_margin=2.0):
...
def fiducial_follow(robot, options, target_fiducial_id):
