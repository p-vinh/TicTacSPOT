...
for i in range(0, general_settings["no_of_markers"]):
    ...
    # 1., Load the image markers[i]["file_name"]
    markers[i]["image_data"] = cv2.imread('cat_logo.jpg', cv2.IMREAD_COLOR)
    ...
    markers[i]["translation_fifo"] = deque(maxlen=int(markers[i]["fifo_length"]))
    markers[i]["rotation_fifo"] = deque(maxlen=int(markers[i]["fifo_length"]))
    markers[i]["2d_centroid_fifo"] = deque(maxlen=int(markers[i]["fifo_length"]))
