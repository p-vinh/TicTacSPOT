...
class TensorFlowObjectDetectionModel:
    def __init__(self, model_path, label_path):
        ...
        print("Available signatures:", self.model.signatures)
        ...
        print("Is signature_fn callable?", callable(self.detect_fn))
        print(type(self.detect_fn))


    def predict(self, image):
        input_tensor = tf.convert_to_tensor(image, dtype=tf.float32) # Ensure the dtype is correct
    ...
    def test_with_dummy_input(self):
    # Create a dummy input tensor with shape [1, height, width, channels]
        dummy_input = tf.random.uniform([1, 640, 480, 3], dtype=tf.float32)
        try:
            # Get the output from the model
            output = self.detect_fn(dummy_input)
            # Print the entire output to inspect its structure
            print("Model output with dummy input:", output)
            # Print the keys of the output dictionary if it is a dictionary
            if isinstance(output, dict):
                print("Output keys:", output.keys())
            else:
                print("Output is not a dictionary. Type:", type(output))
        except Exception as e:
            print("Error during dummy input test:", e)
...
def process_thread(args, request_queue, response_queue):
    ...
    while True:
        ...
        num_detections = int(detections.pop('num_detections', 0))
